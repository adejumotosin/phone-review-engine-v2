...
Original file is located at
    https://colab.research.google.com/drive/1HUGN7Vl2M5flxrAmVPjxnXqmFKIHwk4N
"""
...
class DatabaseManager:
    ...
    def load_database(self):
        """Load database from CSV with comprehensive error handling"""
        try:
            if os.path.exists(self.db_path):
                ...
                if 'rating' in self.df.columns:
                    ...
                    self.df['rating_numeric'] = self.df['rating'].apply(parse_rating)
                ...
                if 'product' not in self.df.columns:
                    for alt_col in ['phone_name', 'model', 'device_name', 'phone']:
                        if alt_col in self.df.columns:
                            self.df['product'] = self.df[alt_col]
                            break
                    else:
                        st.sidebar.error("No product column found")
                        self.df = pd.DataFrame()
                        self.load_status = "Missing product column"
                        return

                self.load_status = f"Loaded {len(self.df)} reviews"
                st.sidebar.success(f"✅ Database: {len(self.df)} reviews")

            else:
                self.load_status = f"File not found"
...
def get_engine():
    ...
    return HybridReviewEngine()

# ... (All the code you provided above) ...
...
def display_word_cloud(analysis: PhoneAnalysis):
    ...
    if not WORDCLOUD_AVAILABLE or not NLTK_AVAILABLE:
        st.warning("Cannot display word cloud: `wordcloud` or `nltk` not available.")
        return

    # We don't have the raw ReviewData list here, so we will generate dummy text
    # based on the analysis results and phone name.

    # In a real app, you would pass the full list of ReviewData to this function.

    # Fallback to generating text from pros/cons and phone name
    pros_text = " ".join([p.split('(')[0] for p in analysis.top_pros])
    cons_text = " ".join([c.split('(')[0] for c in analysis.top_cons])

    word_string = f"{analysis.phone_name} {pros_text} {cons_text} " * 5

    # A better approach would be to extract words from user_quotes and aspect keywords
    ...
    # Add phone name words to stop list
...
try:
    ...
    st.sidebar.warning("⚠️ Matplotlib not available for WordCloud display")
    ...
    if phone_name:
        ...
        if 'analysis_requested' in st.session_state and st.session_state['analysis_requested']:
            with st.container():
                ...
                analysis = run_analysis(st.session_state['phone_name'], st.session_state['force_web'])
